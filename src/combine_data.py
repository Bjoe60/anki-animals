...
def combine_data():
    ...
    df_species = pd.read_csv(os.path.join('data', 'species.csv'))
    df_identification = pd.read_csv(os.path.join('data', 'species with identification.csv'))
    ...
    df_merged = reduce(lambda left, right: pd.merge(left, right, on='eolID', how='left'), [df_species, df_identification, df_translations, df_countries, df_images])
    ...
    # Create tags
    df_merged['countries'] = df_merged['countries'].fillna('')
    df_merged['taxonomy_tag'] = df_merged['countries'] + ' ' + df_merged['taxonomy_tag']
