...
WANTED_TAXA = 'Life|Cellular Organisms|Eukaryota|Opisthokonta|Metazoa|Bilateria|Deuterostomia|Chordata|Vertebrata|Gnathostomata|Osteichthyes|Sarcopterygii|Tetrapoda|Amniota|Synapsida|Therapsida|Cynodontia|Mammalia'
...
def get_species():
	...
	def merge_provider_ids(df, resource_id, id_column):
		df_provider = df_ids[df_ids['resource_id'] == resource_id]
		df = df.merge(df_provider, left_on='eolID', right_on='page_id', how='left')
	...
	df = merge_provider_ids(df, 1177, 'inaturalistID')
	df = merge_provider_ids(df, 1178, 'gbifID')
